...
const calculateExpectedReturn = async (
  ...
  try {
    ...
    const factors = {
      ...
      historicalPerformance: await getHistoricalPerformance(
        artist._id.toString()
      ),
    };
...
const processRevenue = TryCatch(
  async (req: Request<{}, {}, any>, res: Response, next: NextFunction) => {
    const { projectId, source, amount, streamCount, country, platformData } =
      req.body;
...
const processInvestorPayouts = async (revenue: any) => {
  try {
    ...
    for (const investment of investments) {
      ...
      const payoutAmount =
        (revenue.amount * investment.ownershipPercentage) / 100;
